...
if (isset($_GET["f"]) & isset($_GET["id"]))
{
  ...
  if ($f == "ucastnik")
  {
    if (!isset($_GET["yes"]))
    {
      ...
      echo "<div class='prompt'><span class='question'>Smazat úèastníka závodu <i>$name</i> z databáze? Akci nelze vrátit zpìt.</span><br /><a href='delete.php?f=$f&amp;id=$id&amp;yes=1&amp;zavod=$zavod&amp;kategorie=$kategorie'>ANO</a> / <a href='zavod-hrabyne.php'>NE</a></div>";
    }
    else
    {
      $sql = MySQL_Query("DELETE FROM spac_registration WHERE id=$id");
      header("Location: zavod-hrabyne.php");
    }
  }
}
?>
